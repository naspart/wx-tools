...
import com.naspat.pay.constant.WxPayConstants.RefundAccountSource;
...
@AllArgsConstructor
@XStreamAlias("xml")
public class PapPayRefundRequest extends BaseWxPayRequest {
    private static final long serialVersionUID = 8557697181227842601L;

    private static final String[] REFUND_ACCOUNT = new String[]{RefundAccountSource.RECHARGE_FUNDS, RefundAccountSource.UNSETTLED_FUNDS};
    ...
    protected void checkConstraints() throws WxPayException {
        if (StringUtils.isNotBlank(refundAccount)) {
            if (!ArrayUtils.contains(REFUND_ACCOUNT, refundAccount)) {
                throw new WxPayException(String.format("refund_account目前必须为%s其中之一,实际值：%s", Arrays.toString(REFUND_ACCOUNT), refundAccount));
            }
        }
